...
def compute_IoU(pred_box, gt_box):
    ...
    # Calculate the corner of overlapping
...
def IoU(d, g):
    iou_ret = np.zeros((len(d), len(g)))
    for dt in range(len(d)):
        for gt in range(len(g)):
            iou_ret[dt, gt] = compute_IoU(d[dt], g[gt])
    return iou_ret
...
class COCOeval:
    ...
    def evaluate(self):
        ...
        # NOTE! this snippet takes all FNs without consideration of conf threshold, simply all FNs.
        # fns_ids = [img['fnsIds'] for img in E.evalImgs if img is not None and 'fnsIds' in img and not img['gtIgnore'][0]] # in gtIgnore we have 20 entries for all the IoU values, but we want to look at the first value
        if p.compute_fns:
            fns_ids = []
            for e in self.evalImgs:
                if e is not None:
                    for fni in e['fnsIds']:
                        if e['gtIgnore'][np.where(np.asarray(e['gtIds']) == fni)] == 0:
                            if fni not in fns_ids:
                                fns_ids.append(fni)
            fns = self.cocoGt.loadAnns(fns_ids)
            self.fns = fns
...
class Params:
    ...
    def setDetParams(self):
        ...
        self.useCats = 1
        self.compute_fns = True
        self.compute_fps = True
