...
##############################################
# Function that loads a pre-trained OD model #
##############################################
...
def main():
    ...
    #     get_test_acc(model_ori, (1, 28, 28) if "MNIST" in arguments.Config["data"]["dataset"] else (3, 32, 32),
    #                  batch_size=128)
    #     return
    #
    ...
    #     perturb_epsilon = torch.tensor([0.00784313725490196])
    #
    # else:
    ...
    #     format(arguments.Config['model']['name'], arguments.Config["data"]["start"], arguments.Config["data"]["end"],
    ...
    #     get_statistics(model_ori, X, labels, perturb_epsilon, data_min, data_max,
    #                    batch_size=arguments.Config["solver"]["beta-crown"]["batch_size"])
    #
    ...
    #
    # model_ori, data_max, data_min = model_ori.to(arguments.Config["general"]["device"]), data_max.to(
    #     arguments.Config["general"]["device"]), data_min.to(arguments.Config["general"]["device"])
    ...
    #             arguments.Config["general"]["enable_incomplete_verification"] or arguments.Config["general"][
    ...
    #         start_attack = time.time()
    #         if True:
    #             attack_args = {'dataset': attack_dataset, 'model': model_ori, 'x': x, 'max_eps': perturb_eps,
    #                            'data_min': data_min, 'data_max': data_max, 'y': y}
    #             attack_ret, attack_images, attack_margin = pgd_attack(**attack_args)
    #         ret.append([imag_idx, 0, 0, time.time() - start_attack, new_idx, -3, np.inf, np.inf])
    #         if attack_ret:
    #             # Attack success.
    ...
    #             c = torch.eye(arguments.Config["data"]["num_classes"]).type_as(data)[[y]].unsqueeze(1) - torch.eye(
    #                 arguments.Config["data"]["num_classes"]).type_as(data).unsqueeze(0)
    #             lirpa_model, lower_bounds, upper_bounds, masks = saved_bounds[:4]
    #             lirpa_model.build_mip_model(lower_bounds, upper_bounds, arguments.Config["bab"]["timeout"],
    #                                         arguments.Config["solver"]["mip"]["parallel_solvers"],
    #                                         arguments.Config["solver"]["mip"]["solver_threads"])
    #             total_unstable = 0
    ...
    #                     adv_pool.add_adv_images(
    #                         attack_images[:, :, attack_image_target].view((-1, *attack_images.shape[-3:])))
    ...
    #                     best_adv_pattern = [
    #                         torch.stack([adv.activation_pattern[layer_i] for adv in selected_advs], dim=0) for layer_i
    #                         in range(adv_pool.nlayers)]
    #                     print(
    #                         f'Best adv example in pool: {adv_pool.adv_pool[0].obj}, worse {adv_pool.adv_pool[-1].obj}')
    #                     print(f'Target label {target} has {len(coeff)} out of {total_unstable} unstable neurons fixed.')
    #                     attack_ret, solver_results = lirpa_model.update_mip_model_fix_relu([neuron_idx], [coeff],
    #                                                                                        target,
    ...
    #     if not verified_success and (
    #             arguments.Config["general"]["complete_verifier"] == "mip" or arguments.Config["general"][
    #         "complete_verifier"] == "bab-refine"):
    ...
    #                     print(
    #                         f"Initial alpha-CROWN with worst bound {init_global_lb[0].min().item()}. We will run branch and bound.")
    ...
    #                         l, u, nodes, glb_record = bab(model_ori, x, pidx, arguments.Config["specification"]["norm"],
    #                                                       y=y, eps=perturb_eps, data_ub=data_max, data_lb=data_min,
    #                                                       lower_bounds=lower_bounds, upper_bounds=upper_bounds,
    #                                                       reference_slopes=saved_slopes,
    #                                                       attack_images=targeted_attack_images)
    #             else:
    #                 assert arguments.Config["general"]["complete_verifier"] == "bab"  # for MIP and BaB-Refine.
    #                 # Main function to run verification
    #                 l, u, nodes, glb_record = bab(model_ori, x, pidx, arguments.Config["specification"]["norm"], y=y,
    #                                               eps=perturb_eps,
    #                                               data_ub=data_max, data_lb=data_min,
    #                                               attack_images=targeted_attack_images)
    #             time_cost = time.time() - start_inner
    #             print('Image {} label {} verification end, final lower bound {}, upper bound {}, time: {}'.format(
    #                 imag_idx, pidx, l, u, time_cost))
    #             ret.append([imag_idx, l, nodes, time_cost, new_idx, pidx, u,
    #                         attack_margin[pidx] if attack_margin is not None else np.inf])
    ...
    #             format(arguments.Config['model']['name'], arguments.Config["data"]["dataset"],
    #                    arguments.Config["data"]["start"], arguments.Config["data"]["end"]), np.array(attack_success))
    ...
    #         format(arguments.Config['model']['name'], arguments.Config["data"]["dataset"],
    #                arguments.Config["data"]["start"], arguments.Config["data"]["end"], verified_acc,
    #                arguments.Config["bab"]["branching"]["method"]), np.array(verified_failed))
    #
    ...
    #     print("mean time [cnt:{}] (excluding attack success): {}".format(cnt,
    #                                                                      ret[:, 3][ret[:, 5] != -3].sum() / float(cnt)))
    #     if len(attack_success) > 0:
    #         print("mean time [cnt:{}] (including attack success): {}".format(cnt + len(attack_success),
    #                                                                          ret[:, 3].sum() / float(
    #                                                                              cnt + len(attack_success))))
